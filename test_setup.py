...
def test_direct_tool_execution():
    ...
    try:
        ...
        if parsed.get("success"):
            print(f"✅ Tool executed successfully - found {parsed.get('count', 0)} device(s)")
            return True
        else:
            print(f"⚠️  Tool executed but returned error: {parsed.get('error')}")
            return False
